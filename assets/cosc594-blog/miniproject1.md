...
<figure class='text-center'>
  ...
  <figcaption>We have a matrix that can be super easily updated!</figcaption>
</figure>
...
<figure class='text-center'>
  <img width='500px' height='500px' src='/images/cosc594-blog/miniproject1/02-28-green-working.jpg' class='mx-auto'/>
...
The result was a surprisingly responsive (but buggy) movement of the green LED. The player was seemingly wrapping back around, and due to the nature of the input and how the sensor works, you could get really fast sudden movements that basically skipped around sometimes:
...
The solution was to clamp the input value to be in range and further tune the function so that the output behaved as intended and stayed in bounds. Also I made the data type a `uint8_t`, but the values were greater than 255. Oops.

After our meeting, we discussed how our game logic would work, introducing the ideas of shrinking holes in walls and other ways of caling the difficulty of the game as time passed. We planned to implement these features before our next meeting.

#### 03/01/2023
Reed implemented the basic game logic code and I helped debug it over Discord. (add some code blocks for different parts and explain it a bit)

Game ticks blah blah
...
```
and then the return value gets assigned to `matrixData[0]`. This happens every so often, determined by a set of variables we can tune and scale.

Each game tick, the obstacles move one row closer to the player, and implementing this was as simple as moving data from one index to the previous!
```cpp
...
For difficulty scaling we used a system based on blah blah
```cpp
...
The rest of the game logic can be found at the bottom of the page, largely unchanged from the original version.
...
We exchanged many ideas during the session including ... for new obstacles and we want to rethink player input. light sensor is awkward. considered ultrasonic sensor to sense hand tilting

#### 03/06/2023
Today, we decided to completely change the input method. The recent microproject had us use capacitive touch sensors, and we thought that this would make a great control scheme for our game. Adding capacitive touch to Arduino controllers turned out to be very easy&mdash;there is a library called `CapacitiveSensor` that just requires 2 pins and a high Ohm resistor. We were successful with 100kΩ resistors.
...
Also, with two touch pads, there was the possibility of our game supporting a two-player co-op experience, where each person can control a side, which may make navigating the more difficult stages easier with a well-coordinated effort.
...
#### 03/07/2023
Today we originally met to work on the remaining game logic, but during setup we noticed the touch sensing was *far* less responsive compared to yesterday to the point where the game wasn't playable. We tried changing the threshold and number of polls&mdash;and this helped&mdash;but we feared this could have other adverse effects, and we were still completely unsure about the cause. That is, until I remembered overhearing someone after class saying something like "...it changes depending on if your laptop is charging..." and realized this could have been the context of the conversation. Indeed it was. It was on this day we learned that the laptop powering the Arduino needs to be plugged in for reliable touch sensing.

#### 03/08/2023
For the final day, we had 3 main tasks we wanted to complete:

1. Create a score board on the "game over" screen,
2. Create a new obstacle section, as well as tune the difficulty, and
3. Make the touch controlls less finicky.

We decided on a very simple score display. We are already keeping track of the number of ticks progressed through the game, so our score would just be a scaled down version of that.

<figure class='text-center'>
  <img width='500px' height='500px' src='/images/cosc594-blog/miniproject1/03-08-score-test.jpg' class='mx-auto'/>
  <figcaption>The end screen with the score in binary.</figcaption>
</figure>
...
* Each time the obstacle type switches, it will become a little more difficult.
  * For the walls, the distance between the walls will get closer, and the hole size in the wall will also get smaller.
...
Finally, for the controls, it was a lot of trial and error with the number of samples taken as the threshold to cross. Eventually, we found some numbers that make the controls work with very little oddies such as dropping inputs or repeating inputs very quickly.

We felt like the game was in a very good state, so all we had left was to clean up the code to make it look presentable.

During this clean up, we found some issues with the pixels not always wanting to render to the screen, even though we could very clearly see our code update
the matrix data. We fixed this puzzling issue by using a very niche feature of C: the `volatile` keyword.

Our code works by having completely separate rendering and game logic. The game logic updates the matrix, while the rendering logic uses this matrix to draw each LED to the screen. When C is compiled with an opmitizing compiler (like what is happening with `avr-gcc`), "unneeded" writes are dropped. When we write the matrix data, C doesn't know that it will be used by the interrupt handler, so it will drop our writes since we never read from it in the game loop.

We fix this by declaring our matrix with the `volatile` keyword. This forces C to read/write the memory address every single time it is read/written in the source code, in the exact order that it appears. This one simple change fixed every single problem we were still encounting. After some playtesting, we were 100% happy with the final result!
...
ISR(TIMER0_COMPA_vect) {
  ...
  if (isGameRunning) {
    ...
    matrixData[8] = (1 << greenPos) * isGameRunning;
  }
}
